...
public class Import implements TagPlugin {
    ...
    public void doTag(TagPluginContext ctxt) {
        ...
        ctxt.generateJavaSource("    throw new AspTagException(\"The \\\"url\\\" attribute " +
        ...
        if(hasContext){
            ...
            ctxt.generateJavaSource("if((!" + contextName + ".startsWith(\"/\")) " +
                    "|| (!" + urlName + ".startsWith(\"/\"))){");
            ctxt.generateJavaSource("    throw new AspTagException" +
        ...
        if(hasVarReader){
            ...
            ctxt.generateJavaSource("            throw new AspTagException(status + \" \" + " + urlName + ");");
            ctxt.generateJavaSource("        }");
            ctxt.generateJavaSource("    }");
            ...
            ctxt.generateJavaSource("        throw new AspTagException(\"Relative &lt;import&gt; from non-HTTP request not allowed\");");
            ctxt.generateJavaSource("    }");
            ...
            if(hasContext){
                ctxt.generateJavaSource("        throw new AspTagException(\"Unable to get RequestDispatcher for Context: \\\" \"+" + contextName + "+\" \\\" and URL: \\\" \" +" + urlName + "+ \" \\\". Verify values and/or enable cross context access.\");");
            }else{
                ctxt.generateJavaSource("        throw new AspTagException(\"Unable to get RequestDispatcher for  URL: \\\" \" +" + urlName + "+ \" \\\". Verify values and/or enable cross context access.\");");
            }
            ctxt.generateJavaSource("    }");
            ...
            ctxt.generateJavaSource("    if(" + requestDispatcherName + " == null) throw new AspTagException(org.apache.jasper.tagplugins.jstl.Util.stripSession("+urlName+"));");
            ...
            ctxt.generateJavaSource("        throw new AspException(ex);");
            ...
            ctxt.generateJavaSource("            throw new AspException(ex);");
            ctxt.generateJavaSource("        else");
            ctxt.generateJavaSource("            throw new AspException(rc);");
            ctxt.generateJavaSource("    }");
            
            //validate the response status
            ctxt.generateJavaSource("    if(" + irwName + ".getStatus() < 200 || " + irwName + ".getStatus() > 299){");
            ctxt.generateJavaSource("        throw new AspTagException(" + irwName + ".getStatus()+\" \" + org.apache.jasper.tagplugins.jstl.Util.stripSession(" + urlName + "));");
            ctxt.generateJavaSource("    }");
        ...
        //if the varReader is not specified 
            ...
            ctxt.generateJavaSource("        " + tempReaderName + " = new java.io.InputStreamReader(" + inputStreamName + "," + charSetName + ");");
            ctxt.generateJavaSource("    }catch(Exception ex){");
            //ctxt.generateJavaSource("        throw new AspTagException(ex.toString());");
            ctxt.generateJavaSource("        " + tempReaderName + " = new java.io.InputStreamReader(" + inputStreamName + ",org.apache.jasper.tagplugins.jstl.Util.DEFAULT_ENCODING);");
            ctxt.generateJavaSource("    }");
