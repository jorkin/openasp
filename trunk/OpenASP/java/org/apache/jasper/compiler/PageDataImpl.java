...
/**
 * An implementation of <tt>javax.servlet.asp.tagext.PageData</tt> which
 ...
 * top-level asp:root and those of the asp:root elements of any included
 * pages, and adds them to the asp:root element of the XML view.
 * In addition, any taglib directives are converted into xmlns: attributes and
 * added to the asp:root element of the XML view.
 * This pass ignores any nodes other than AspRoot and TaglibDirective.
 *
 * During the second pass, the SecondPassVisitor produces the XML view, using
 * the combined asp:root attributes determined in the first pass and any
 * remaining pages nodes (this pass ignores any AspRoot and TaglibDirective
 * nodes).
 *
 * @author Jan Luehe
 */
class PageDataImpl extends PageData implements TagConstants {
    ...
    /*
     * First-pass Visitor for AspRoot nodes (representing asp:root elements)
     * and TablibDirective nodes, ignoring any other nodes.
     *
     * The purpose of this Visitor is to collect the attributes of the
     * top-level asp:root and those of the asp:root elements of any included
     * pages, and add them to the asp:root element of the XML view.
     * In addition, this Visitor converts any taglib directives into xmlns:
     * attributes and adds them to the asp:root element of the XML view.
     */
    static class FirstPassVisitor
        ...
        public void visit(Node.Root n) throws JasperException {
            ...
            if (n == root) {
                 ...
                 *
                 * Add
                 *   xmlns:asp="http://java.sun.com/JSP/Page"
                 * attribute only if not already present.
                 */
                if (!JSP_URI.equals(rootAttrs.getValue("xmlns:asp"))) {
                    rootAttrs.addAttribute("", "", "xmlns:asp", "CDATA",
                                           JSP_URI);
                }

                if (pageInfo.isAspPrefixHijacked()) {
                    /*
                     * 'asp' prefix has been hijacked, that is, bound to a
                     * namespace other than the JSP namespace. This means that
                     * when adding an 'id' attribute to each element, we can't
                     * use the 'asp' prefix. Therefore, create a new prefix 
                     * (one that is unique across the translation unit) for use
                     * by the 'id' attribute, and bind it to the JSP namespace
                     */
                    aspIdPrefix += "asp";
                    while (pageInfo.containsPrefix(aspIdPrefix)) {
                        aspIdPrefix += "asp";
                    }
                    rootAttrs.addAttribute("", "", "xmlns:" + aspIdPrefix,
                                           "CDATA", JSP_URI);
                }
        ...
        /*
         * Converts taglib directive into "xmlns:..." attribute of asp:root
         * element.
         */
        @Override
     ...
     * each element a unique asp:id attribute.
     */
    static class SecondPassVisitor extends Node.Visitor
        ...
        // Current value of asp:id attribute
        private int aspId;
        ...
        public SecondPassVisitor(Node.Root root, StringBuilder buf,
                                 Compiler compiler, String aspIdPrefix) {
            this.root = root;
            this.buf = buf;
            this.compiler = compiler;
            this.aspIdPrefix = aspIdPrefix;
        }
    ...
    public void visit(Node.Root n) throws JasperException {
        ...
        /*
         * Visits asp:root element of JSP page in XML syntax.
         *
         * Any nested asp:root elements (from pages included via an
    ...
    public void visit(Node.ELExpression n) throws JasperException {
            if (!n.getRoot().isXmlSyntax()) {
                ...
                buf.append(" ");
                buf.append(aspIdPrefix);
                buf.append(":id=\"");
                buf.append(aspId++).append("\">");
            }
            buf.append("${");
            buf.append(AspUtil.escapeXml(n.getText()));
            buf.append("}");
            if (!n.getRoot().isXmlSyntax()) {
    ...
    public void visit(Node.UninterpretedTag n) throws JasperException {
        ...
        @Override
    public void visit(Node.AspText n) throws JasperException {
            appendTag(n);
        }
    ...
    public void visit(Node.TemplateText n) throws JasperException {
             ...
             * create a asp:text element for it (JSP 5.3.2).
             */
        ...
        private void appendTag(Node n, boolean addDefaultNS)
            ...
            buf.append("  ").append(aspIdPrefix).append(":id").append("=\"");
            buf.append(aspId++).append("\"\n");
        ...
        private void appendPageDirective(Node.PageDirective n) {
            ...
            for (int i=0; i<len; i++) {
                ...
                buf.append(AspUtil.getExprInXml(value)).append("\"\n");
            }
            if (n.getImports().size() > 0) {
                ...
                while (iter.hasNext()) {
                    ...
                    buf.append(AspUtil.getExprInXml(iter.next()));
                }
                buf.append("\"\n");
            }
            buf.append("/>\n");
        ...
        private void appendText(String text, boolean createAspTextElement) {
            if (createAspTextElement) {
                buf.append("<").append(JSP_TEXT_ACTION);
                buf.append("\n");
