...
public class StatusTransformer {
    ...
    protected static void writeContext(PrintWriter writer, 
        ...
        if (mode == 0){
            ...
            ObjectName queryAspMonitor = new ObjectName
                (objectName.getDomain() + ":type=AspMonitor,WebModule=" +
                 webModuleName + ",*");
            Set<ObjectName> aspMonitorONs =
                mBeanServer.queryNames(queryAspMonitor, null);
            ...
            if (aspMonitorONs != null) {
                writeAspMonitor(writer, aspMonitorONs, mBeanServer, mode);
            }
            writer.print("</p>");
     ...
     */
    public static void writeAspMonitor(PrintWriter writer,
                                       Set<ObjectName> aspMonitorONs,
                                       MBeanServer mBeanServer,
                                       int mode)
            throws Exception {
        ...
        int aspCount = 0;
        int aspReloadCount = 0;

        Iterator<ObjectName> iter = aspMonitorONs.iterator();
        while (iter.hasNext()) {
            ObjectName aspMonitorON = iter.next();
            Object obj = mBeanServer.getAttribute(aspMonitorON, "aspCount");
            aspCount += ((Integer) obj).intValue();
            obj = mBeanServer.getAttribute(aspMonitorON, "aspReloadCount");
            aspReloadCount += ((Integer) obj).intValue();
        }

        if (mode == 0) {
            writer.print("<br>");
            writer.print(" JSPs loaded: ");
            writer.print(aspCount);
            writer.print(" JSPs reloaded: ");
            writer.print(aspReloadCount);
        } else if (mode == 1) {
