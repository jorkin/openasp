...
%>
  <h2>服务器概况</h2>
	<table border=0 width=500 cellspacing=1 cellpadding=3>
	  ...
	  <tr>
		<td>服务器脚本引擎</td><td><%=ScriptEngine & "/"& ScriptEngineMajorVersion &"."&ScriptEngineMinorVersion&"."& ScriptEngineBuildVersion %></td>
	  </tr>
	  <%getsysinfo()
	  '获得服务器数据
	  %>
...
ObjTest(strClass)
  ...
  Else
	if VerObj="" or isnull(VerObj) then 
	  Verobj1="-无法取得该组件版本-"
	Else
	  Verobj1="-组件版本是：" & VerObj & "-"
	End If
	Response.Write "-<font class=fonts>恭喜！服务器支持 " & strclass & " 组件" & verobj1 & "</font>-"
  End If
end if
%>
  </td></tr>
