...
import org.apache.jasper.compiler.AspUtil;
 ...
 */
public class AspCompilationContext {

    private final Log log = LogFactory.getLog(AspCompilationContext.class); // must not be static

    protected Map<String, JarResource> tagFileJarUrls;

    protected String className;
    protected String aspUri;
    ...
    protected Compiler aspCompiler;
    ...
    protected URLClassLoader aspLoader;
    ...
    // aspURI _must_ be relative to the context
    public AspCompilationContext(String aspUri,
                                 Options options,
                                 ServletContext context,
                                 AspServletWrapper jsw,
                                 AspRuntimeContext rctxt) {
        this.aspUri = canonicalURI(aspUri);
        this.options = options;
        this.jsw = jsw;
        this.context = context;

        this.baseURI = aspUri.substring(0, aspUri.lastIndexOf('/') + 1);
    ...
    public AspCompilationContext(String tagfile,
                                 TagInfo tagInfo,
                                 Options options,
                                 ServletContext context,
                                 AspServletWrapper jsw,
    ...
    public ClassLoader getAspLoader() {
        if( aspLoader == null ) {
            aspLoader = new JasperLoader
            (new URL[] {baseUrl},
                    getClassLoader(),
    ...
    public Compiler createCompiler() {
        ...
        if (options.getCompilerClassName() != null) {
            aspCompiler = createCompiler(options.getCompilerClassName());
        } else {
            if (options.getCompiler() == null) {
                aspCompiler = createCompiler("org.apache.jasper.compiler.JDTCompiler");
                if (aspCompiler == null) {
                    aspCompiler = createCompiler("org.apache.jasper.compiler.AntCompiler");
                }
            } else {
                aspCompiler = createCompiler("org.apache.jasper.compiler.AntCompiler");
                if (aspCompiler == null) {
        ...
        if (aspCompiler == null) {
            throw new IllegalStateException(Localizer.getMessage("asp.error.compiler"));
        }
        aspCompiler.init(this, jsw);
        return aspCompiler;
    }
    ...
    protected Compiler createCompiler(String className) {
        ...
        try {
            ...
            log.warn(Localizer.getMessage("asp.error.compiler"), e);
        } catch (IllegalAccessException e) {
            log.warn(Localizer.getMessage("asp.error.compiler"), e);
        } catch (NoClassDefFoundError e) {
            if (log.isDebugEnabled()) {
                log.debug(Localizer.getMessage("asp.error.compiler"), e);
            }
     ...
     * AspCompilationContext was created is packaged, or null if this
     * AspCompilationContext does not correspond to a tag file, or if the
    ...
    public String getServletClassName() {
        ...
        if (isTagFile) {
            ...
            int iSep = aspUri.lastIndexOf('/') + 1;
            className = AspUtil.makeJavaIdentifier(aspUri.substring(iSep));
        }
        return className;
    }
     ...
     */
    @Deprecated
    public long getAspLastModified() {
        long result = -1;
        URLConnection uc = null;
        try {
            URL aspUrl = getResource(getAspFile());
            if (aspUrl == null) {
                incrementRemoved();
                return result;
            }
            uc = aspUrl.openConnection();
            ...
            if (log.isDebugEnabled()) {
                log.debug(Localizer.getMessage(
                        "asp.error.lastModified", getAspFile()), e);
            }
            result = -1;
        } finally {
            if (uc != null) {
    ...
    public Long getLastModified(String resource) {
        ...
        URLConnection uc = null;
        try {
            URL aspUrl = getResource(resource);
            if (aspUrl == null) {
                incrementRemoved();
    ...
    protected String getDerivedPackageName() {
        if (derivedPackageName == null) {
            int iSep = aspUri.lastIndexOf('/');
            derivedPackageName = (iSep > 0) ?
                    AspUtil.makeJavaPackage(aspUri.substring(1,iSep)) : "";
        }
    ...
    public AspRuntimeContext getRuntimeContext() {
        return rctxt;
    }
    ...
    public void compile() throws JasperException, FileNotFoundException {
        createCompiler();
        if (aspCompiler.isOutDated()) {
            if (isRemoved()) {
                throw new FileNotFoundException(aspUri);
            }
            try {
                aspCompiler.removeGeneratedFiles();
                aspLoader = null;
                aspCompiler.compile();
                jsw.setReload(true);
                ...
                JasperException je = new JasperException(
                            Localizer.getMessage("asp.error.unable.compile"),
                            ex);
                // Cache compilation exception
    ...
    public Class<?> load() throws JasperException {
        try {
            ...
            String name = getFQCN();
            servletClass = aspLoader.loadClass(name);
        } catch (ClassNotFoundException cex) {
            throw new JasperException(Localizer.getMessage("asp.error.unable.load"),
                                      cex);
        } catch (Exception ex) {
            throw new JasperException(Localizer.getMessage("asp.error.unable.compile"),
                                      ex);
        }
        removed = 0;
        return servletClass;
        ...
        if (isTagFile()) {
            ...
            try {
                ...
                outputDir = base.getAbsolutePath() + File.separator + path +
                    File.separator;
                if (!makeOutputDir()) {
                    throw new IllegalStateException(Localizer.getMessage("asp.error.outputfolder"));
                }
            } catch (MalformedURLException e) {
                throw new IllegalStateException(Localizer.getMessage("asp.error.outputfolder"), e);
            }
    }
