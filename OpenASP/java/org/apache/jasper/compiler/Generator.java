...
import javax.servlet.asp.tagext.TagVariableInfo;
...
import org.apache.jasper.runtime.AspRuntimeLibrary;
...
class Generator {
    ...
    private static final String VAR_INSTANCEMANAGER =
        System.getProperty("org.apache.jasper.compiler.Generator.VAR_INSTANCEMANAGER", "_asp_instancemanager");
     ...
     * used in asp:getProperty action to be previously "introduced"
    ...
    private String createAspId() {
        if (this.aspIdPrefix == null) {
            ...
            sb.append('_');
            this.aspIdPrefix = sb.toString();
        }
        return this.aspIdPrefix + (this.aspId++);
    }
    ...
    private void compileTagHandlerPoolList(Node.Nodes page)
        ...
        class TagHandlerPoolVisitor extends Node.Visitor {
            ...
            private String createTagHandlerPoolName(String prefix,
                ...
                poolName.append("_aspx_tagPool_").append(prefix).append('_')
                        .append(shortName);
                ...
                return AspUtil.makeJavaIdentifier(poolName.toString());
            }
        }
    ...
    private void declareTemporaryScriptingVars(Node.Nodes page)
        ...
        class ScriptingVarVisitor extends Node.Visitor {
            ...
            public void visit(Node.CustomTag n) throws JasperException {
                ...
                // "_aspx_" + varName + "_" + nestingLevel variables when we are
                // inside a AspFragment.
                ...
                if (n.getCustomNestingLevel() > 0) {
                    ...
                    if (varInfos.length > 0) {
                        for (int i = 0; i < varInfos.length; i++) {
                            ...
                            String tmpVarName = "_aspx_" + varName + "_"
    ...
    /**
     * Generates the _aspInit() method for instantiating the tag handler pools.
     * For tag file, _aspInit has to be invoked manually, and the ServletConfig
    ...
    private void generateInit() {
        ...
        if (ctxt.isTagFile()) {
            out.printil("private void _aspInit(javax.servlet.ServletConfig config) {");
        } else {
            out.printil("public void _aspInit() {");
        }
        ...
        out.print(" = _aspxFactory.getAspApplicationContext(");
        if (ctxt.isTagFile()) {
            out.print("config");
        } else {
            out.print("getServletConfig()");
    ...
    /**
     * Generates the _aspDestroy() method which is responsible for calling the
     * release() method on every tag handler in any of the tag handler pools.
     */
    private void generateDestroy() {

        out.printil("public void _aspDestroy() {");
        out.pushIndent();
    ...
    private void genPreambleStaticInitializers() {
        out.printil("private static final javax.servlet.asp.AspFactory _aspxFactory =");
        out.printil("        javax.servlet.asp.AspFactory.getDefaultFactory();");
        out.println();

        // Static data for getDependants()
        out.printil("private static java.util.Map<java.lang.String,java.lang.Long> _aspx_dependants;");
        out.println();
        ...
        if (!dependants.isEmpty()) {
            out.printil("static {");
            out.pushIndent();
            out.printin("_aspx_dependants = new java.util.HashMap<java.lang.String,java.lang.Long>(");
            out.print("" + dependants.size());
            out.println(");");
            while (iter.hasNext()) {
                Entry<String,Long> entry = iter.next();
                out.printin("_aspx_dependants.put(\"");
    ...
    private void genPreambleMethods() {
        ...
        out.pushIndent();
        out.printil("return _aspx_dependants;");
        out.popIndent();
        out.printil("}");
        out.println();
    ...
    private void generatePreamble(Node.Nodes page) throws JasperException {
        ...
        out.printin("    implements org.apache.jasper.runtime.AspSourceDependent");
        ...
        out.printil("final javax.servlet.asp.PageContext pageContext;");

        if (pageInfo.isSession())
            out.printil("javax.servlet.http.HttpSession session = null;");

        if (pageInfo.isErrorPage()) {
            out.printil("java.lang.Throwable exception = org.apache.jasper.runtime.AspRuntimeLibrary.getThrowable(request);");
        ...
        out.printil("javax.servlet.asp.AspWriter out = null;");
        out.printil("final java.lang.Object page = this;");

        out.printil("javax.servlet.asp.AspWriter _aspx_out = null;");
        out.printil("javax.servlet.asp.PageContext _aspx_page_context = null;");
        out.println();
        ...
        if (ctxt.getOptions().isXpoweredBy()) {
            out.printil("response.addHeader(\"X-Powered-By\", \"ASP/2.1\");");
        }

        out.printil("pageContext = _aspxFactory.getPageContext(this, request, response,");
        out.printin("\t\t\t");
        ...
        out.println(");");
        out.printil("_aspx_page_context = pageContext;");
    ...
    private void generateXmlProlog(Node.Nodes page) {
         ...
         * 'omit-xml-declaration' attribute of <asp:output> action is set to
         * "no" or "false" - ASP document without a <asp:root>
         */
        String omitXmlDecl = pageInfo.getOmitXmlDecl();
        if ((omitXmlDecl != null && !AspUtil.booleanValue(omitXmlDecl))
                || (omitXmlDecl == null && page.getRoot().isXmlSyntax()
                        && !pageInfo.hasAspRoot() && !ctxt.isTagFile())) {
    ...
    class GenerateVisitor extends Node.Visitor {
         ...
         */
        private String attributeValue(Node.AspAttribute attr, boolean encode,
                Class<?> expectedType) {
            ...
            if (attr.isExpression()) {
                if (encode) {
                    return "org.apache.jasper.runtime.AspRuntimeLibrary.URLEncode(String.valueOf("
                            + v + "), request.getCharacterEncoding())";
                }
                return v;
            } else if (attr.isELInterpreterInput()) {
                v = AspUtil.interpreterCall(this.isTagFile, v, expectedType,
                        attr.getEL().getMapName(), false);
                if (encode) {
                    return "org.apache.jasper.runtime.AspRuntimeLibrary.URLEncode("
                            + v + ", request.getCharacterEncoding())";
                }
                return v;
        ...
        private void printParams(Node n, String pageParam, boolean literal)
            ...
            class ParamVisitor extends Node.Visitor {
                ...
                public void visit(Node.ParamAction n) throws JasperException {
                    ...
                    out.print(separator);
                    out.print(" + ");
                    out.print("org.apache.jasper.runtime.AspRuntimeLibrary."
        ...
        public void visit(Node.ELExpression n) throws JasperException {
            ...
            if (!pageInfo.isELIgnored() && (n.getEL() != null)) {
                out.printil("out.write("
                        + AspUtil.interpreterCall(this.isTagFile, n.getType() +
        ...
        public void visit(Node.IncludeAction n) throws JasperException {
            ...
            Node.AspAttribute page = n.getPage();
            ...
            if (page.isNamedAttribute()) {
                // If the page for asp:include was specified via
                // asp:attribute, first generate code to evaluate
                // that body.
            ...
            // asp:attribute, prepare those values first.
            Node aspBody = findAspBody(n);
            if (aspBody != null) {
                prepareParams(aspBody);
            } else {
                prepareParams(n);
            }

            out.printin("org.apache.jasper.runtime.AspRuntimeLibrary.include(request, response, "
                    + pageParam);
         ...
         * a Named Attribute (&lt;asp:attribute&gt;), generate the code to evaluate
         * those bodies first.
         * <p>
        ...
        /**
         * Finds the &lt;asp:body&gt; subelement of the given parent node. If not
         * found, null is returned.
         */
        private Node.AspBody findAspBody(Node parent) {
            Node.AspBody result = null;

            Node.Nodes subelements = parent.getBody();
            for (int i = 0; (subelements != null) && (i < subelements.size()); i++) {
                Node n = subelements.getNode(i);
                if (n instanceof Node.AspBody) {
                    result = (Node.AspBody) n;
                    break;
                }
            }
        ...
        public void visit(Node.ForwardAction n) throws JasperException {
            ...
            String pageParam;
            if (page.isNamedAttribute()) {
                // If the page for asp:forward was specified via
                // asp:attribute, first generate code to evaluate
                // that body.
                pageParam = generateNamedAttributeValue(page
                        .getNamedAttributeNode());
            ...
            out.printin("_aspx_page_context.forward(");
            out.print(pageParam);
            printParams(n, pageParam, page.isLiteral());
            out.println(");");
            if (isTagFile || isFragment) {
                out.printil("throw new javax.servlet.asp.SkipPageException();");
            } else {
        ...
        public void visit(Node.GetProperty n) throws JasperException {
            ...
            if (beanInfo.checkVariable(name)) {
                ...
                java.lang.reflect.Method meth = AspRuntimeLibrary
                        .getReadMethod(bean, property);
                String methodName = meth.getName();
                out.printil("out.write(org.apache.jasper.runtime.AspRuntimeLibrary.toString("
                        + "((("
                        + beanName
                        + ")_aspx_page_context.findAttribute("
                        + "\""
                        + name + "\"))." + methodName + "())));");
                ...
                out.printil("out.write(org.apache.jasper.runtime.AspRuntimeLibrary.toString"
                        + "(org.apache.jasper.runtime.AspRuntimeLibrary.handleGetProperty"
                        + "(_aspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
                ...
                msg.append(" asp:getProperty for bean with name '");
                msg.append(name);
                msg.append(
        ...
        public void visit(Node.SetProperty n) throws JasperException {
            ...
            Node.AspAttribute value = n.getValue();

            n.setBeginJavaLine(out.getJavaLine());

            if ("*".equals(property)) {
                out.printil("org.apache.jasper.runtime.AspRuntimeLibrary.introspect("
                        + "_aspx_page_context.findAttribute("
                        + "\""
                        + name + "\"), request);");
            } else if (value == null) {
                ...
                out.printil("org.apache.jasper.runtime.AspRuntimeLibrary.introspecthelper("
                        + "_aspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
                ...
                out.printil("org.apache.jasper.runtime.AspRuntimeLibrary.handleSetProperty("
                        + "_aspx_page_context.findAttribute(\""
                        + name
                        + "\"), \"" + property + "\",");
                ...
                // <asp:useBean>, much as the code here does for
                // getProperty.)

                // The following holds true for the arguments passed to
                // AspRuntimeLibrary.handleSetPropertyExpression():
                ...
                out.printil("org.apache.jasper.runtime.AspRuntimeLibrary.handleSetPropertyExpression("
                        + "_aspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
                ...
                out.printin("org.apache.jasper.runtime.AspRuntimeLibrary.introspecthelper("
                        + "_aspx_page_context.findAttribute(\""
                        + name
                        + "\"), \"" + property + "\", ");
                out.print(attributeValue(value, false, null));
        ...
        public void visit(Node.UseBean n) throws JasperException {
            ...
            Node.AspAttribute beanName = n.getBeanName();
            ...
            if (klass != null) {
                try {
                    ...
                    if (ctxt.getOptions()
                        ...
                        err.aspError(n, "asp.error.invalid.bean", klass);
                    }
            ...
            // page scope
            String scopename = "javax.servlet.asp.PageContext.PAGE_SCOPE"; // Default to page
            String lock = null;

            if ("request".equals(scope)) {
                scopename = "javax.servlet.asp.PageContext.REQUEST_SCOPE";
            } else if ("session".equals(scope)) {
                scopename = "javax.servlet.asp.PageContext.SESSION_SCOPE";
                lock = "session";
            } else if ("application".equals(scope)) {
                scopename = "javax.servlet.asp.PageContext.APPLICATION_SCOPE";
                lock = "application";
            }
            ...
            out.print(" = (");
            out.print(type);
            out.print(") _aspx_page_context.getAttribute(");
            ...
            if (klass == null && beanName == null) {
                 ...
                 */
                out.printin("_aspx_page_context.setAttribute(");
                out.print(quote(name));
                out.print(", ");
                out.print(name);
                out.print(", ");
        ...
        public void visit(Node.PlugIn n) throws JasperException {
            ...
            /**
             * A visitor to handle &lt;asp:param&gt; in a plugin
             */
            class ParamVisitor extends Node.Visitor {
            ...
            String name = n.getTextAttribute("name");
            Node.AspAttribute height = n.getHeight();
            Node.AspAttribute width = n.getWidth();
            ...
            Node.AspBody aspBody = findAspBody(n);
            if (aspBody != null) {
                Node.Nodes subelements = aspBody.getBody();
                if (subelements != null) {
                    for (int i = 0; i < subelements.size(); i++) {
            ...
            /*
             * generate a <param> for each <asp:param> in the plugin body
             */
            if (n.getBody() != null)
            ...
            /*
             * Generate a 'attr = "value"' for each <asp:param> in plugin body
             */
            if (n.getBody() != null)
        ...
        public void visit(Node.CustomTag n) throws JasperException {
            ...
            String baseVar = createTagVarName(n.getQName(), n.getPrefix(), n
                    .getLocalName());
            String tagEvalVar = "_aspx_eval_" + baseVar;
            String tagHandlerVar = "_aspx_th_" + baseVar;
            String tagPushBodyCountVar = "_aspx_push_body_count_" + baseVar;
            ...
            if (ci.isScriptless() && !ci.hasScriptingVars()) {
                ...
                String tagMethod = "_aspx_meth_" + baseVar;
                ...
                out.print("(");
                if (parent != null) {
                    out.print("javax.servlet.asp.tagext.AspTag ");
                    out.print(parent);
                    out.print(", ");
                }
                out.print("javax.servlet.asp.PageContext _aspx_page_context");
                if (pushBodyCountVar != null) {
                    out.print(", int[] ");
                    out.print(pushBodyCountVar);
                }
                ...
                if (!isTagFile) {
                    out.printil("javax.servlet.asp.PageContext pageContext = _aspx_page_context;");
                }
                out.printil("javax.servlet.asp.AspWriter out = _aspx_page_context.getOut();");
        ...
        public void visit(Node.UninterpretedTag n) throws JasperException {
            ...
            if (attrs != null) {
                Node.AspAttribute[] aspAttrs = n.getAspAttributes();
                for (int i = 0; i < attrs.getLength(); i++) {
                    out.print(" ");
                    out.print(attrs.getQName(i));
                    out.print("=");
                    if (aspAttrs[i].isELInterpreterInput()) {
                        out.print("\\\"\" + ");
                        out.print(attributeValue(aspAttrs[i], false,
                                String.class));
                        out.print(" + \"\\\"");
                    } else {
                        out.print(DOUBLE_QUOTE);
        ...
        public void visit(Node.AspElement n) throws JasperException {
            ...
            Node.AspAttribute[] attrs = n.getAspAttributes();
            for (int i = 0; attrs != null && i < attrs.length; i++) {
                String value = null;
                String nvp = null;
                if (attrs[i].isNamedAttribute()) {
                    NamedAttribute attr = attrs[i].getNamedAttributeNode();
                    Node.AspAttribute omitAttr = attr.getOmit();
                    String omit;
                    if (omitAttr == null) {
                        omit = "false";
                    } else {
            ...
            // Does the <asp:element> have nested tags other than
            // <asp:attribute>
            ...
            if (ctxt.getOptions().genStringAsCharArray()) {
                ...
                while (textIndex < textLength) {
                    ...
                    if (charArrayName == null) {
                        charArrayName = "_aspx_char_array_" + charArrayCount++;
        ...
        public void visit(Node.AspBody n) throws JasperException {
            if (n.getBody() != null) {
                if (isSimpleTagHandler) {
                    ...
                    out.print(".setAspBody(");
                    generateAspFragment(n, simpleTagHandlerVar);
                    out.println(");");
                } else {
                    visitBody(n);
                }
        ...
        public void visit(Node.InvokeAction n) throws JasperException {
            ...
            // page
            out.printil("((org.apache.jasper.runtime.AspContextWrapper) this.aspContext).syncBeforeInvoke();");
            String varReaderAttr = n.getTextAttribute("varReader");
            String varAttr = n.getTextAttribute("var");
            if (varReaderAttr != null || varAttr != null) {
                out.printil("_aspx_sout = new java.io.StringWriter();");
            } else {
                out.printil("_aspx_sout = null;");
            }
            ...
            if (varReaderAttr != null || varAttr != null) {
                ...
                if (varReaderAttr != null) {
                    ...
                    out.print(", new java.io.StringReader(_aspx_sout.toString())");
                } else {
                    out.print(quote(varAttr));
                    out.print(", _aspx_sout.toString()");
                }
                if (scopeName != null) {
                    out.print(", ");
        ...
        public void visit(Node.DoBodyAction n) throws JasperException {
            ...
            out.printil("if (getAspBody() != null)");
            out.pushIndent();
            out.printil("getAspBody().invoke(_aspx_sout);");
            out.popIndent();
            ...
            // Restore EL context
            out.printil("aspContext.getELContext().putContext(javax.servlet.asp.AspContext.class,getAspContext());");
        ...
        public void visit(Node.AttributeGenerator n) throws JasperException {
            ...
            Node.AspAttribute[] attrs = tag.getAspAttributes();
            for (int i = 0; attrs != null && i < attrs.length; i++) {
        ...
        private void generateCustomStart(Node.CustomTag n,
            ...
            if (isPoolingEnabled && !(n.implementsAspIdConsumer())) {
            ...
            // AspIdConsumer (after context has been set)
            if (n.implementsAspIdConsumer()) {
                out.printin(tagHandlerVar);
                out.print(".setAspId(\"");
                out.print(createAspId());
                out.println("\");");
            }
            ...
            if (!n.hasEmptyBody()) {
                out.printin("if (");
                out.print(tagEvalVar);
                out.println(" != javax.servlet.asp.tagext.Tag.SKIP_BODY) {");
                out.pushIndent();
                ...
                if (n.implementsBodyTag()) {
                    out.printin("if (");
                    out.print(tagEvalVar);
                    out.println(" != javax.servlet.asp.tagext.Tag.EVAL_BODY_INCLUDE) {");
                    // Assume EVAL_BODY_BUFFERED
                    out.pushIndent();
                    out.printil("out = _aspx_page_context.pushBody();");
                    if (n.implementsTryCatchFinally()) {
                    ...
                    out.printin(tagHandlerVar);
                    out.println(".setBodyContent((javax.servlet.asp.tagext.BodyContent) out);");
                    out.printin(tagHandlerVar);
        ...
        private void generateCustomEnd(Node.CustomTag n, String tagHandlerVar,
            ...
            if (!n.hasEmptyBody()) {
                if (n.implementsIterationTag()) {
                    ...
                    out.printil("if (evalDoAfterBody != javax.servlet.asp.tagext.BodyTag.EVAL_BODY_AGAIN)");
                    out.pushIndent();
                    out.printil("break;");
                    out.popIndent();
                ...
                if (n.implementsBodyTag()) {
                    ...
                    out.println(" != javax.servlet.asp.tagext.Tag.EVAL_BODY_INCLUDE) {");
                    out.pushIndent();
                    out.printil("out = _aspx_page_context.popBody();");
                    if (n.implementsTryCatchFinally()) {
                        out.printin(tagPushBodyCountVar);
                        out.println("[0]--;");
                    } else if (pushBodyCountVar != null) {
            ...
            out.printin("if (");
            out.print(tagHandlerVar);
            out.println(".doEndTag() == javax.servlet.asp.tagext.Tag.SKIP_PAGE) {");
            out.pushIndent();
            if (!n.implementsTryCatchFinally()) {
                if (isPoolingEnabled && !(n.implementsAspIdConsumer())) {
            ...
            if (n.implementsTryCatchFinally()) {
                out.popIndent(); // try
                out.printil("} catch (java.lang.Throwable _aspx_exception) {");
                out.pushIndent();
                ...
                out.printin(tagHandlerVar);
                out.println(".doCatch(_aspx_exception);");
                out.popIndent();
                out.printil("} finally {");
                out.pushIndent();
                out.printin(tagHandlerVar);
        ...
        private void generateCustomDoTag(Node.CustomTag n,
            ...
            // Set the body
            if (findAspBody(n) == null) {
                /*
                 * Encapsulate body of custom tag invocation in AspFragment and
                 * pass it to tag handler's setAspBody(), unless tag body is
                 * empty
                 */
                if (!n.hasEmptyBody()) {
                    out.printin(tagHandlerVar);
                    out.print(".setAspBody(");
                    generateAspFragment(n, tagHandlerVar);
                    out.println(");");
                }
            } else {
                /*
                 * Body of tag is the body of the <asp:body> element. The visit
                 * method for that element is going to encapsulate that
                 * element's body in a AspFragment and pass it to the tag
                 * handler's setAspBody()
                 */
        ...
        private void declareScriptingVars(Node.CustomTag n, int scope) {
            if (isFragment) {
                ...
                // AspFragment, because a fragment is always scriptless.
                return;
            }
        ...
        private void saveScriptingVars(Node.CustomTag n, int scope) {
            ...
            if (isFragment) {
                ...
                // Note, that AspContextWrapper.syncFoo() methods will take
                // care of saving/ restoring/ sync'ing of AspContext attributes.
                return;
            }
            ...
            if (varInfos.length > 0) {
                for (int i = 0; i < varInfos.length; i++) {
                    if (varInfos[i].getScope() == scope) {
                        ...
                        out.print(") _aspx_page_context.findAttribute(");
        ...
        private String getAspContextVar() {
            if (this.isTagFile) {
                return "this.getAspContext()";
            }
            return "_aspx_page_context";
        }
        ...
        private String createTagVarName(String fullName, String prefix,
            ...
            return AspUtil.makeJavaIdentifier(varName);
        }

        @SuppressWarnings("null")
        private String evaluateAttribute(TagHandlerInfo handlerInfo,
                Node.AspAttribute attr, Node.CustomTag n, String tagHandlerVar)
                throws JasperException {

            String attrValue = attr.getValue();
            if (attrValue == null) {
                if (attr.isNamedAttribute()) {
                    if (n.checkIfAttributeIsAspFragment(attr.getName())) {
                        // XXX - no need to generate temporary variable here
                        attrValue = generateNamedAttributeAspFragment(attr
            ...
            if (attr.isDynamic()) {
                ...
                if (m == null) {
                    err.aspError(n, "asp.error.unable.to_find_method", attr
                            .getName());
                }
            ...
            if (attr.isExpression()) {
                ...
                if (attr.isDeferredInput()
                    ...
                    sb.append("new org.apache.jasper.el.AspValueExpression(");
                    ...
                    sb.append(',');
                    sb.append(AspUtil.toJavaSourceTypeFromTld(attr.getExpectedTypeName()));
                    sb.append("))");
                    ...
                    sb.append("new org.apache.jasper.el.AspMethodExpression(");
                    sb.append(quote(mark));
                    sb.append(',');
                    sb.append(getExpressionFactoryVar());
                    ...
                    for (int i = 0; i < p.length; i++) {
                        sb.append(AspUtil.toJavaSourceTypeFromTld(p[i]));
                        sb.append(',');
                    }
                    if (p.length > 0) {
                    ...
                    String mapName = (attr.getEL() != null) ? attr.getEL()
                            .getMapName() : null;
                    attrValue = AspUtil.interpreterCall(this.isTagFile, attrValue,
                            c[0], mapName, false);
                }
            } else {
                attrValue = convertString(c[0], attrValue, localName,
        ...
        private void generateSetters(Node.CustomTag n, String tagHandlerVar,
            ...
            if (simpleTag) {
                ...
                out.printin(tagHandlerVar);
                if (aliasMapVar == null) {
                    out.println(".setAspContext(_aspx_page_context);");
                } else {
                    out.print(".setAspContext(_aspx_page_context, ");
                    out.print(aliasMapVar);
                    out.println(");");
                }
            } else {
                out.printin(tagHandlerVar);
                out.println(".setPageContext(_aspx_page_context);");
            }
            ...
            if (isTagFile && parent == null) {
                out.printin(tagHandlerVar);
                out.print(".setParent(");
                out.print("new javax.servlet.asp.tagext.TagAdapter(");
                out.print("(javax.servlet.asp.tagext.SimpleTag) this ));");
            } else if (!simpleTag) {
                out.printin(tagHandlerVar);
                out.print(".setParent(");
                if (parent != null) {
                    if (isSimpleTagParent) {
                        out.print("new javax.servlet.asp.tagext.TagAdapter(");
                        out.print("(javax.servlet.asp.tagext.SimpleTag) ");
                        out.print(parent);
                        out.println("));");
                    } else {
                        out.print("(javax.servlet.asp.tagext.Tag) ");
                        out.print(parent);
                        out.println(");");
                    }
                } else {
         ...
         * asp:attribute standard action), and false otherwise
         */
        private String convertString(Class<?> c, String s, String attrName,
            ...
            if (propEditorClass != null) {
                ...
                return "("
                        + className
                        + ")org.apache.jasper.runtime.AspRuntimeLibrary.getValueFromBeanInfoPropertyEditor("
                        + className + ".class, \"" + attrName + "\", " + quoted
                        + ", " + propEditorClass.getCanonicalName() + ".class)";
            } else if (c == String.class) {
                return quoted;
            } else if (c == boolean.class) {
                return AspUtil.coerceToPrimitiveBoolean(s, isNamedAttribute);
            } else if (c == Boolean.class) {
                return AspUtil.coerceToBoolean(s, isNamedAttribute);
            } else if (c == byte.class) {
                return AspUtil.coerceToPrimitiveByte(s, isNamedAttribute);
            } else if (c == Byte.class) {
                return AspUtil.coerceToByte(s, isNamedAttribute);
            } else if (c == char.class) {
                return AspUtil.coerceToChar(s, isNamedAttribute);
            } else if (c == Character.class) {
                return AspUtil.coerceToCharacter(s, isNamedAttribute);
            } else if (c == double.class) {
                return AspUtil.coerceToPrimitiveDouble(s, isNamedAttribute);
            } else if (c == Double.class) {
                return AspUtil.coerceToDouble(s, isNamedAttribute);
            } else if (c == float.class) {
                return AspUtil.coerceToPrimitiveFloat(s, isNamedAttribute);
            } else if (c == Float.class) {
                return AspUtil.coerceToFloat(s, isNamedAttribute);
            } else if (c == int.class) {
                return AspUtil.coerceToInt(s, isNamedAttribute);
            } else if (c == Integer.class) {
                return AspUtil.coerceToInteger(s, isNamedAttribute);
            } else if (c == short.class) {
                return AspUtil.coerceToPrimitiveShort(s, isNamedAttribute);
            } else if (c == Short.class) {
                return AspUtil.coerceToShort(s, isNamedAttribute);
            } else if (c == long.class) {
                return AspUtil.coerceToPrimitiveLong(s, isNamedAttribute);
            } else if (c == Long.class) {
                return AspUtil.coerceToLong(s, isNamedAttribute);
                ...
                return "("
                        + className
                        + ")org.apache.jasper.runtime.AspRuntimeLibrary.getValueFromPropertyEditorManager("
                        + className + ".class, \"" + attrName + "\", " + quoted
                        + ")";
            }
        }
        ...
        /**
         * Generates anonymous AspFragment inner class which is passed as an
         * argument to SimpleTag.setAspBody().
         */
        private void generateAspFragment(Node n, String tagHandlerVar)
                throws JasperException {
            ...
            // generate a low-overhead AspFragment that just echoes its
            ...
            if (pushBodyCountVar != null) {
                ...
                pushBodyCountVar = "_aspx_push_body_count";
            }
            visitBody(n);
            out = outSave;
            parent = tmpParent;
            ...
            // XXX - Need to change pageContext to aspContext if
            ...
            out.print("new " + fragmentHelperClass.getClassName() + "( "
                    + fragment.getId() + ", _aspx_page_context, "
            ...
            if (body != null) {
                ...
                if (!templateTextOptimization) {
                    ...
                    out.printil("java.lang.String " + varName + " = "
                            + "((javax.servlet.asp.tagext.BodyContent)"
                            + "out).getString();");
                    out.printil("out = _aspx_page_context.popBody();");
                }
            } else {
        ...
        /**
         * Similar to generateNamedAttributeValue, but create a AspFragment
         * instead.
         *
         * @param n
         ...
         */
        public String generateNamedAttributeAspFragment(Node.NamedAttribute n,
                String tagHandlerVar) throws JasperException {
            String varName = n.getTemporaryVariableName();

            out.printin("javax.servlet.asp.tagext.AspFragment " + varName
                    + " = ");
            generateAspFragment(n, tagHandlerVar);
            out.println(";");
    ...
    private static void generateLocalVariables(ServletWriter out, Node n)
        ...
        if (n instanceof Node.CustomTag) {
        ...
        } else if (n instanceof Node.AspBody) {
            ci = ((Node.AspBody) n).getChildInfo();
            ...
            // err.getString(
            // "asp.error.internal.unexpected_node_type" ) );
        }

        if (ci.hasUseBean()) {
            out.printil("javax.servlet.http.HttpSession session = _aspx_page_context.getSession();");
            out.printil("javax.servlet.ServletContext application = _aspx_page_context.getServletContext();");
        }
        if (ci.hasUseBean() || ci.hasIncludeAction() || ci.hasSetProperty()
                || ci.hasParamAction()) {
            out.printil("javax.servlet.http.HttpServletRequest request = (javax.servlet.http.HttpServletRequest)_aspx_page_context.getRequest();");
        }
        if (ci.hasIncludeAction()) {
            out.printil("javax.servlet.http.HttpServletResponse response = (javax.servlet.http.HttpServletResponse)_aspx_page_context.getResponse();");
        }
    }
    ...
    private void generatePostamble() {
        ...
        out.pushIndent();
        out.printil("if (!(t instanceof javax.servlet.asp.SkipPageException)){");
        out.pushIndent();
        out.printil("out = _aspx_out;");
        out.printil("if (out != null && out.getBufferSize() != 0)");
        out.pushIndent();
        out.printil("try { out.clearBuffer(); } catch (java.io.IOException e) {}");
        out.popIndent();
        out.printil("if (_aspx_page_context != null) _aspx_page_context.handlePageException(t);");
        ...
        out.printil("} finally {");
        out.pushIndent();
        out.printil("_aspxFactory.releasePageContext(_aspx_page_context);");
        out.popIndent();
        out.printil("}");
    ...
    Generator(ServletWriter out, Compiler compiler) {
         ...
         * page directive, the _aspInit() method of the generated servlet class
         * will not be called (it is only called for those generated servlets
         * that extend HttpAspBase, the default), causing the tag handler pools
    ...
    private void generateCommentHeader() {
        out.println("/*");
        out.println(" * Generated by the Jasper component of OpenASP");
    ...
    private void generateTagHandlerPreamble(JasperTagInfo tagInfo,
        ...
        out.printil("    extends javax.servlet.asp.tagext.SimpleTagSupport");
        out.printin("    implements org.apache.jasper.runtime.AspSourceDependent");
        if (tagInfo.hasDynamicAttributes()) {
            out.println(",");
            out.printin("               javax.servlet.asp.tagext.DynamicAttributes");
        }
        out.println(" {");
        out.println();
        out.pushIndent();
        ...
        out.printil("private javax.servlet.asp.AspContext aspContext;");

        // Declare writer used for storing result of fragment/body invocation
        // if 'varReader' or 'var' attribute is specified
        out.printil("private java.io.Writer _aspx_sout;");

        // Class variable declarations
        genPreambleClassVariableDeclarations();

        generateSetAspContext(tagInfo);
        ...
        out.printil("public void doTag() throws javax.servlet.asp.AspException, java.io.IOException {");
         ...
         * implicit object in tag files. Declare _aspx_page_context, so we can
         * share the code generator with JSPs.
         */
        out.printil("javax.servlet.asp.PageContext _aspx_page_context = (javax.servlet.asp.PageContext)aspContext;");

        // Declare implicit objects.
        out.printil("javax.servlet.http.HttpServletRequest request = "
                + "(javax.servlet.http.HttpServletRequest) _aspx_page_context.getRequest();");
        out.printil("javax.servlet.http.HttpServletResponse response = "
                + "(javax.servlet.http.HttpServletResponse) _aspx_page_context.getResponse();");
        out.printil("javax.servlet.http.HttpSession session = _aspx_page_context.getSession();");
        out.printil("javax.servlet.ServletContext application = _aspx_page_context.getServletContext();");
        out.printil("javax.servlet.ServletConfig config = _aspx_page_context.getServletConfig();");
        out.printil("javax.servlet.asp.AspWriter out = aspContext.getOut();");
        out.printil("_aspInit(config);");

        // set current AspContext on ELContext
        out.printil("aspContext.getELContext().putContext(javax.servlet.asp.AspContext.class,aspContext);");
    ...
    private void generateTagHandlerPostamble(TagInfo tagInfo) {
        ...
        out.pushIndent();
        out.printil("if( t instanceof javax.servlet.asp.SkipPageException )");
        out.printil("    throw (javax.servlet.asp.SkipPageException) t;");
        ...
        out.printil("if( t instanceof javax.servlet.asp.AspException )");
        out.printil("    throw (javax.servlet.asp.AspException) t;");
        out.printil("throw new javax.servlet.asp.AspException(t);");
        out.popIndent();
        out.printil("} finally {");
        out.pushIndent();
        ...
        // restore nested AspContext on ELContext
        out.printil("aspContext.getELContext().putContext(javax.servlet.asp.AspContext.class,super.getAspContext());");

        out.printil("((org.apache.jasper.runtime.AspContextWrapper) aspContext).syncEndTagFile();");
        if (isPoolingEnabled && !tagHandlerPoolNames.isEmpty()) {
            out.printil("_aspDestroy();");
        }
        out.popIndent();
        out.printil("}");
    ...
    private void generateTagHandlerAttributes(TagInfo tagInfo) {
        ...
        if (tagInfo.hasDynamicAttributes()) {
            out.printil("private java.util.HashMap _aspx_dynamic_attrs = new java.util.HashMap();");
        }
        ...
        for (int i = 0; i < attrInfos.length; i++) {
            ...
            if (attrInfos[i].isFragment()) {
                out.print("javax.servlet.asp.tagext.AspFragment ");
            } else {
                out.print(AspUtil.toJavaSourceType(attrInfos[i].getTypeName()));
                out.print(" ");
            }
            out.print(AspUtil.makeJavaIdentifierForAttribute(
                    attrInfos[i].getName()));
            out.println(";");
        }
        out.println();
        ...
        for (int i = 0; i < attrInfos.length; i++) {
            String javaName =
                AspUtil.makeJavaIdentifierForAttribute(attrInfos[i].getName());
            ...
            out.println(";");
            if (ctxt.isTagFile()) {
                // Tag files should also set aspContext attributes
                out.printin("aspContext.setAttribute(\"");
    ...
    /*
     * Generate setter for AspContext so we can create a wrapper and store both
     ...
     */
    private void generateSetAspContext(TagInfo tagInfo) {
        ...
        if (aliasSeen) {
            out.printil("public void setAspContext(javax.servlet.asp.AspContext ctx, java.util.Map aliasMap) {");
        } else {
            out.printil("public void setAspContext(javax.servlet.asp.AspContext ctx) {");
        }
        out.pushIndent();
        out.printil("super.setAspContext(ctx);");
        out.printil("java.util.ArrayList _aspx_nested = null;");
        out.printil("java.util.ArrayList _aspx_at_begin = null;");
        out.printil("java.util.ArrayList _aspx_at_end = null;");
        ...
        for (int i = 0; i < tagVars.length; i++) {
            ...
            case VariableInfo.NESTED:
                if (!nestedSeen) {
                    out.printil("_aspx_nested = new java.util.ArrayList();");
                    nestedSeen = true;
                }
                out.printin("_aspx_nested.add(");
                break;

            case VariableInfo.AT_BEGIN:
                if (!atBeginSeen) {
                    out.printil("_aspx_at_begin = new java.util.ArrayList();");
                    atBeginSeen = true;
                }
                out.printin("_aspx_at_begin.add(");
                break;

            case VariableInfo.AT_END:
                if (!atEndSeen) {
                    out.printil("_aspx_at_end = new java.util.ArrayList();");
                    atEndSeen = true;
                }
                out.printin("_aspx_at_end.add(");
                break;
            } // switch
        ...
        if (aliasSeen) {
            out.printil("this.aspContext = new org.apache.jasper.runtime.AspContextWrapper(ctx, _aspx_nested, _aspx_at_begin, _aspx_at_end, aliasMap);");
        } else {
            out.printil("this.aspContext = new org.apache.jasper.runtime.AspContextWrapper(ctx, _aspx_nested, _aspx_at_begin, _aspx_at_end, null);");
        }
        out.popIndent();
        out.printil("}");
        out.println();
        out.printil("public javax.servlet.asp.AspContext getAspContext() {");
        out.pushIndent();
        out.printil("return this.aspContext;");
        out.popIndent();
        out.printil("}");
    }
     ...
     * javax.servlet.asp.tagext.DynamicAttributes.setDynamicAttribute() method,
     * which saves each dynamic attribute that is passed in so that a scoped
     * variable can later be created for it.
     */
    public void generateSetDynamicAttribute() {
        out.printil("public void setDynamicAttribute(java.lang.String uri, java.lang.String localName, java.lang.Object value) throws javax.servlet.asp.AspException {");
        out.pushIndent();
        /*
        ...
        out.pushIndent();
        out.printil("_aspx_dynamic_attrs.put(localName, value);");
        out.popIndent();
        out.popIndent();
        out.printil("}");
    }
        ...
        for (int i = 0; i < attrInfos.length; i++) {
            ...
            if (attrInfos[i].isDeferredValue() || attrInfos[i].isDeferredMethod()) {
                ...
                if (!variableMapperVar) {
                    out.printil("javax.el.VariableMapper _el_variablemapper = aspContext.getELContext().getVariableMapper();");
    ...
    private static class TagHandlerInfo {
        ...
        TagHandlerInfo(Node n, Class<?> tagHandlerClass,
            ...
            try {
                ...
                err.aspError(n, "asp.error.introspect.taghandler",
    ...
    private static class FragmentHelperClass {
        ...
        public void generatePreamble() {
            ...
            out.printil("    extends "
                    + "org.apache.jasper.runtime.AspFragmentHelper");
            out.printil("{");
            out.pushIndent();
            out.printil("private javax.servlet.asp.tagext.AspTag _aspx_parent;");
            out.printil("private int[] _aspx_push_body_count;");
            out.println();
            out.printil("public " + className
                    + "( int discriminator, javax.servlet.asp.AspContext aspContext, "
                    + "javax.servlet.asp.tagext.AspTag _aspx_parent, "
                    + "int[] _aspx_push_body_count ) {");
            out.pushIndent();
            out.printil("super( discriminator, aspContext, _aspx_parent );");
            out.printil("this._aspx_parent = _aspx_parent;");
            out.printil("this._aspx_push_body_count = _aspx_push_body_count;");
            out.popIndent();
            out.printil("}");
        }
        ...
        throws JasperException {
            ...
            out.println(result.getId() + "( " + "javax.servlet.asp.AspWriter out ) ");
            out.pushIndent();
            // Note: Throwable required because methods like _aspx_meth_*
            // throw Throwable.
            out.printil("throws java.lang.Throwable");
            out.popIndent();
            out.printil("{");
        ...
        public void generatePostamble() {
            ...
            out.pushIndent();
            out.printil("throws javax.servlet.asp.AspException");
            out.popIndent();
            out.printil("{");
            out.pushIndent();
            out.printil("javax.servlet.asp.AspWriter out = null;");
            ...
            out.pushIndent();
            out.printil("out = this.aspContext.pushBody(writer);");
            out.popIndent();
            out.printil("} else {");
            out.pushIndent();
            out.printil("out = this.aspContext.getOut();");
            out.popIndent();
            out.printil("}");
            out.printil("try {");
            out.pushIndent();
            out.printil("Object _aspx_saved_AspContext = this.aspContext.getELContext().getContext(javax.servlet.asp.AspContext.class);");
            out.printil("this.aspContext.getELContext().putContext(javax.servlet.asp.AspContext.class,this.aspContext);");
            ...
            // restore nested AspContext on ELContext
            out.printil("aspContext.getELContext().putContext(javax.servlet.asp.AspContext.class,_aspx_saved_AspContext);");

            out.popIndent();
            out.printil("}"); // try
            out.printil("catch( java.lang.Throwable e ) {");
            out.pushIndent();
            out.printil("if (e instanceof javax.servlet.asp.SkipPageException)");
            out.printil("    throw (javax.servlet.asp.SkipPageException) e;");
            out.printil("throw new javax.servlet.asp.AspException( e );");
            out.popIndent();
            out.printil("}"); // catch
            out.printil("finally {");
            out.pushIndent();
            ...
            out.printil("if( writer != null ) {");
            out.pushIndent();
            out.printil("this.aspContext.popBody();");
            out.popIndent();
            out.printil("}");
