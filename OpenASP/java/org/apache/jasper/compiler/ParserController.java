...
class ParserController implements TagConstants {
     ...
     * Constructor
     */
    public ParserController(AspCompilationContext ctxt, Compiler compiler) {
        this.ctxt = ctxt; 
        this.compiler = compiler;
        this.err = compiler.getErrorDispatcher();
    }

    public AspCompilationContext getAspCompilationContext () {
        return ctxt;
    }
    ...
    throws FileNotFoundException, JasperException, IOException {
        ...
        String aspConfigPageEnc = getAspConfigPageEncoding(absFileName);

        // Figure out what type of JSP document and encoding type we are
        // dealing with
        determineSyntaxAndEncoding(absFileName, jarFile, aspConfigPageEnc);
        ...
        if ((isXml && isEncodingSpecifiedInProlog) || isBomPresent) {
             ...
             * identical.
             */
            if (aspConfigPageEnc != null && !aspConfigPageEnc.equals(sourceEnc)
                    && (!aspConfigPageEnc.startsWith("UTF-16")
                            || !sourceEnc.startsWith("UTF-16"))) {
                err.aspError("asp.error.prolog_config_encoding_mismatch",
                        sourceEnc, aspConfigPageEnc);
            }
        }
        ...
        if (isXml) {
            ...
            // InputStream for aspx page is created and properly closed in
            // AspDocumentParser.
            parsedPage = AspDocumentParser.parse(this, absFileName,
                    jarFile, parent,
            ...
            try {
                inStreamReader = AspUtil.getReader(absFileName, sourceEnc,
                        jarFile, ctxt, err, skip);
                AspReader aspReader = new AspReader(ctxt, absFileName,
                        sourceEnc, inStreamReader,
                        err);
                parsedPage = Parser.parse(this, aspReader, parent, isTagFile,
     ...
     * a asp-property-group in web.xml, and if so, returns the value of the
     * <page-encoding> element.
     *
     * @param absFileName The URI to match
     *
     * @return The value of the <page-encoding> attribute of the 
     * asp-property-group with matching URL pattern
     */
    private String getAspConfigPageEncoding(String absFileName)
    throws JasperException {

        AspConfig aspConfig = ctxt.getOptions().getAspConfig();
        AspConfig.AspProperty aspProperty
            = aspConfig.findAspProperty(absFileName);
        return aspProperty.getPageEncoding();
    }
    ...
    throws JasperException, IOException {
         ...
         * the ".aspx" suffix, or the enclosing file (for included resources)
         */
        ...
        AspConfig aspConfig = ctxt.getOptions().getAspConfig();
        AspConfig.AspProperty aspProperty = aspConfig.findAspProperty(
                absFileName);
        if (aspProperty.isXml() != null) {
            // If <is-xml> is specified in a <asp-property-group>, it is used.
            isXml = AspUtil.booleanValue(aspProperty.isXml());
            isExternal = true;
        } else if (absFileName.endsWith(".aspx")
        ...
        if (isExternal && !isXml) {
            // JSP (standard) syntax. Use encoding specified in asp-config
            // if provided.
            sourceEnc = aspConfigPageEnc;
            if (sourceEnc != null) {
                return;
            ...
            if (!isXml && sourceEnc.equals("UTF-8")) {
                 ...
                 * <asp:root>) are identical in either encoding (both UTF-8
        ...
        if (isXml) {
            ...
            // ".aspx" suffix), so we're done.
            return;
        }

        /*
         * At this point, 'isExternal' or 'isXml' is FALSE.
         * Search for asp:root action, in order to determine if we're dealing 
         ...
         */
        AspReader aspReader = null;
        try {
            aspReader = new AspReader(ctxt, absFileName, sourceEnc, jarFile,
                    err);
        } catch (FileNotFoundException ex) {
            throw new JasperException(ex);
        }
        aspReader.setSingleFile(true);
        Mark startMark = aspReader.mark();
        if (!isExternal) {
            aspReader.reset(startMark);
            if (hasAspRoot(aspReader)) {
                if (revert) {
                    sourceEnc = "UTF-8";
                }
                isXml = true;
        ...
        if (!isBomPresent) {
            sourceEnc = aspConfigPageEnc;
            if (sourceEnc == null) {
                sourceEnc = getPageEncodingForAspSyntax(aspReader, startMark);
                if (sourceEnc == null) {
     ...
     */
    private String getPageEncodingForAspSyntax(AspReader aspReader,
            Mark startMark)
    throws JasperException {
         ...
         * <%@ tag %>, <asp:directive.page > or <asp:directive.tag >.
         */
        while (true) {
            if (aspReader.skipUntil("<") == null) {
                break;
            }
            // If this is a comment, skip until its end
            if (aspReader.matches("%--")) {
                if (aspReader.skipUntil("--%>") == null) {
            ...
            boolean isDirective = aspReader.matches("%@");
            if (isDirective) {
                aspReader.skipSpaces();
            }
            else {
                isDirective = aspReader.matches("asp:directive.");
            }
            if (!isDirective) {
                continue;
            }
            ...
            if (aspReader.matches("tag ") || aspReader.matches("page")) {

                aspReader.skipSpaces();
                Attributes attrs = Parser.parseAttributes(this, aspReader);
     ...
     */
    private boolean hasAspRoot(AspReader reader) throws JasperException {
