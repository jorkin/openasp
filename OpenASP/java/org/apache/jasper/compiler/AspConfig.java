...
/**
 * Handles the asp-config element in WEB_INF/web.xml.  This is used
...
public class AspConfig {
    ...
    // Logger
    private final Log log = LogFactory.getLog(AspConfig.class);

    private Vector<AspPropertyGroup> aspProperties = null;
    ...
    private AspProperty defaultAspProperty;

    public AspConfig(ServletContext ctxt) {
        this.ctxt = ctxt;
    }
    ...
    private void processWebDotXml() throws JasperException {
        ...
        try {
            ...
            TreeNode aspConfig = webApp.findChild("asp-config");
            if (aspConfig == null) {
                return;
            }

            aspProperties = new Vector<AspPropertyGroup>();
            Iterator<TreeNode> aspPropertyList =
                aspConfig.findChildren("asp-property-group");
            while (aspPropertyList.hasNext()) {

                TreeNode element = aspPropertyList.next();
                ...
                // Add one AspPropertyGroup for each URL Pattern.  This makes
                ...
                for( int p = 0; p < urlPatterns.size(); p++ ) {
                    ...
                    if (urlPattern.indexOf('*') < 0) {
                        ...
                        // pattern must be "*", or of the form "*.asp"
                        if (file.equals("*")) {
                            extension = "*";
                        ...
                        if ((path == null && (extension == null || isStar))
                            ...
                            if (log.isWarnEnabled()) {
                                log.warn(Localizer.getMessage(
                                        "asp.warning.bad.urlpattern.propertygroup",
                                        urlPattern));
                            }
                            continue;
                        }
                    ...
                    AspProperty property = new AspProperty(isXml,
                            elIgnored,
                            scriptingInvalid,
                            pageEncoding,
                            includePrelude,
                    ...
                    AspPropertyGroup propertyGroup =
                        new AspPropertyGroup(path, extension, property);

                    aspProperties.addElement(propertyGroup);
                }
            }
        } catch (Exception ex) {
    ...
    private void init() throws JasperException {
        ...
        if (!initialized) {
            synchronized (this) {
                if (!initialized) {
                    processWebDotXml();
                    defaultAspProperty = new AspProperty(defaultIsXml,
                            defaultIsELIgnored,
     ...
     */
    private AspPropertyGroup selectProperty(AspPropertyGroup prev,
            AspPropertyGroup curr) {
        if (prev == null) {
            return curr;
        }
     ...
     * @return a AspProperty indicating the best match, or some default.
     */
    public AspProperty findAspProperty(String uri) throws JasperException {

        init();

        // JSP Configuration settings do not apply to tag files
        if (aspProperties == null || uri.endsWith(".tag")
                || uri.endsWith(".tagx")) {
            return defaultAspProperty;
        }
        ...
        AspPropertyGroup isXmlMatch = null;
        AspPropertyGroup elIgnoredMatch = null;
        AspPropertyGroup scriptingInvalidMatch = null;
        AspPropertyGroup pageEncodingMatch = null;
        AspPropertyGroup deferedSyntaxAllowedAsLiteralMatch = null;
        AspPropertyGroup trimDirectiveWhitespacesMatch = null;
        AspPropertyGroup defaultContentTypeMatch = null;
        AspPropertyGroup bufferMatch = null;
        AspPropertyGroup errorOnUndeclaredNamespaceMatch = null;

        Iterator<AspPropertyGroup> iter = aspProperties.iterator();
        while (iter.hasNext()) {

            AspPropertyGroup jpg = iter.next();
            AspProperty jp = jpg.getAspProperty();
        ...
        if (isXmlMatch != null) {
            isXml = isXmlMatch.getAspProperty().isXml();
        }
        if (elIgnoredMatch != null) {
            isELIgnored = elIgnoredMatch.getAspProperty().isELIgnored();
        }
        if (scriptingInvalidMatch != null) {
            isScriptingInvalid =
                scriptingInvalidMatch.getAspProperty().isScriptingInvalid();
        }
        if (pageEncodingMatch != null) {
            pageEncoding = pageEncodingMatch.getAspProperty().getPageEncoding();
        }
        if (deferedSyntaxAllowedAsLiteralMatch != null) {
            isDeferedSyntaxAllowedAsLiteral =
                deferedSyntaxAllowedAsLiteralMatch.getAspProperty().isDeferedSyntaxAllowedAsLiteral();
        }
        if (trimDirectiveWhitespacesMatch != null) {
            isTrimDirectiveWhitespaces =
                trimDirectiveWhitespacesMatch.getAspProperty().isTrimDirectiveWhitespaces();
        }
        if (defaultContentTypeMatch != null) {
            defaultContentType =
                defaultContentTypeMatch.getAspProperty().getDefaultContentType();
        }
        if (bufferMatch != null) {
            buffer = bufferMatch.getAspProperty().getBuffer();
        }
        if (errorOnUndeclaredNamespaceMatch != null) {
            errorOnUndelcaredNamespace =
                errorOnUndeclaredNamespaceMatch.getAspProperty().isErrorOnUndeclaredNamespace();
        }

        return new AspProperty(isXml, isELIgnored, isScriptingInvalid,
    ...
    /**
     * To find out if an uri matches an url pattern in asp config.  If so,
     * then the uri is a JSP page.  This is used primarily for aspc.
     */
    public boolean isAspPage(String uri) throws JasperException {

        init();
        if (aspProperties == null) {
            return false;
        }
    ...
    public static class AspPropertyGroup {
        private String path;
        private String extension;
        private AspProperty aspProperty;

        AspPropertyGroup(String path, String extension,
                AspProperty aspProperty) {
            this.path = path;
            this.extension = extension;
            this.aspProperty = aspProperty;
        }
        ...
        public AspProperty getAspProperty() {
            return aspProperty;
        }
    }

    public static class AspProperty {
        ...
        public AspProperty(String isXml, String elIgnored,
