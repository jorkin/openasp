...
class ImplicitTagLibraryInfo extends TagLibraryInfo {
     ...
     * Constructor.
     */
    public ImplicitTagLibraryInfo(AspCompilationContext ctxt,
            ParserController pc,
            PageInfo pi,
            String prefix,
            String tagdir,
        ...
        aspversion = JSP_VERSION;

        if (!tagdir.startsWith(WEB_INF_TAGS)) {
            err.aspError("asp.error.invalid.tagdir", tagdir);
        }
                        ...
                        if (in != null) {
                            ...
                            while (list.hasNext()) {
                                ...
                                if ("tlibversion".equals(tname) // JSP 1.1
                                    ...
                                    err.aspError("asp.error.invalid.implicit", path);
                                }
                            }
                            try {
                                double version = Double.parseDouble(this.aspversion);
                                if (version < 2.0) {
                                    err.aspError("asp.error.invalid.implicit.version", path);
                                }
        ...
        if (tagFile == null) {
            ...
            try {
                tagInfo = TagFileProcessor.parseTagFileDirectives(pc,
                        shortName,
                        path,
                        pc.getAspCompilationContext().getTagFileJarResource(path),
                        this);
