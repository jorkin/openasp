...
/**
 * AspReader is an input buffer for the JSP parser. It should allow
...
class AspReader {
     ...
     * Logger.
     */
    private final Log log = LogFactory.getLog(AspReader.class);
     ...
     */
    private AspCompilationContext context;
    ...
    /**
     * Set to true when using the AspReader on a single file where we read up
     * to the end and reset to the beginning many times.
     * (as in ParserController.figureOutAspDocument()).
     */
     ...
     */
    public AspReader(AspCompilationContext ctxt,
                     String fname,
                     String encoding,
                     JarFile jarFile,
                     ErrorDispatcher err)
        ...
        this(ctxt, fname, encoding,
             AspUtil.getReader(fname, encoding, jarFile, ctxt, err),
             err);
    }
    ...
    /**
     * @return JSP compilation context with which this AspReader is 
     * associated
     */
    AspCompilationContext getAspCompilationContext() {
        return context;
    }
    ...
    String parseToken(boolean quoted) throws JasperException {
        ...
        if (quoted) {
            if (ch == '"' || ch == '\'') {
                ...
                if (ch == -1) {
                    err.aspError(mark(), "asp.error.quotes.unterminated");
                }
            } else {
                err.aspError(mark(), "asp.error.attr.quoted");
            }
        } else {
            if (!isDelimiter()) {
    ...
    private void pushFile(String file, String encoding, 
        ...
        if (fileid == -1) {
            ...
            err.aspError("asp.error.file.already.registered", file);
        }
        ...
        try {
            ...
            popFile();
            err.aspError("asp.error.file.cannot.read", file);
        } finally {
            if (reader != null) {
                try {
                    reader.close();
    ...
    private boolean popFile() throws JasperException {
        ...
        // Is stack created ? (will happen if the Asp file we're looking at is
        // missing.
        ...
        if (currFileId < -1) {
            err.aspError("asp.error.file.not.registered", fName);
        }
