...
public class XMLEncodingDetector {
    ...
    {
        InputStream inStream = AspUtil.getInputStream(fname, jarFile, ctxt,
                                                      err);
    ...
    private Reader createReader(InputStream inputStream, String encoding,
        ...
        if (ENCODING.equals("ISO-10646-UCS-4")) {
            if (isBigEndian != null) {
                ...
                err.aspError("asp.error.xml.encodingByteOrderUnsupported",
                             encoding);
            }
        }
        ...
        if (!validIANA || (fAllowJavaEncodings && !validJava)) {
            err.aspError("asp.error.xml.encodingDeclInvalid", encoding);
    ...
    private void scanXMLDeclOrTextDecl(boolean scanningTextDecl,
            ...
            switch (state) {
                    ...
                    if (name == fVersionSymbol) {
                        if (!sawSpace) {
                            reportFatalError(scanningTextDecl
                                       ? "asp.error.xml.spaceRequiredBeforeVersionInTextDecl"
                                       : "asp.error.xml.spaceRequiredBeforeVersionInXMLDecl",
                                             null);
                        }
                        ...
                        if (!version.equals("1.0")) {
                            ...
                            err.aspError("asp.error.xml.versionNotSupported",
                                         version);
                        }
                    } else if (name == fEncodingSymbol) {
                        if (!scanningTextDecl) {
                            err.aspError("asp.error.xml.versionInfoRequired");
                        }
                        if (!sawSpace) {
                            reportFatalError(scanningTextDecl
                                      ? "asp.error.xml.spaceRequiredBeforeEncodingInTextDecl"
                                      : "asp.error.xml.spaceRequiredBeforeEncodingInXMLDecl",
                                             null);
                        }
                        ...
                        if (scanningTextDecl) {
                            err.aspError("asp.error.xml.encodingDeclRequired");
                        }
                        else {
                            err.aspError("asp.error.xml.versionInfoRequired");
                        }
                ...
                case STATE_ENCODING: {
                    if (name == fEncodingSymbol) {
                        ...
                        if (!sawSpace) {
                            err.aspError("asp.error.xml.spaceRequiredBeforeStandalone");
                        }
                        standalone = fString.toString();
                        state = STATE_DONE;
                        if (!standalone.equals("yes") && !standalone.equals("no")) {
                            err.aspError("asp.error.xml.sdDeclInvalid");
                        }
                    } else {
                        err.aspError("asp.error.xml.encodingDeclRequired");
                    }
                ...
                default: {
                    err.aspError("asp.error.xml.noMorePseudoAttributes");
                }
            }
            sawSpace = skipSpaces();
        }
        ...
        if (scanningTextDecl && state != STATE_DONE) {
            err.aspError("asp.error.xml.morePseudoAttributes");
        }
        ...
        // end
        if (!skipChar('?')) {
            err.aspError("asp.error.xml.xmlDeclUnterminated");
        }
        if (!skipChar('>')) {
            err.aspError("asp.error.xml.xmlDeclUnterminated");
    ...
    public String scanPseudoAttribute(boolean scanningTextDecl, 
        ...
        if (name == null) {
            err.aspError("asp.error.xml.pseudoAttrNameExpected");
        }
        skipSpaces();
        if (!skipChar('=')) {
            reportFatalError(scanningTextDecl ?
                             "asp.error.xml.eqRequiredInTextDecl"
                             : "asp.error.xml.eqRequiredInXMLDecl",
                             name);
        }
        skipSpaces();
        int quote = peekChar();
        if (quote != '\'' && quote != '"') {
            reportFatalError(scanningTextDecl ?
                             "asp.error.xml.quoteRequiredInTextDecl"
                             : "asp.error.xml.quoteRequiredInXMLDecl" ,
                             name);
        }
        scanChar();
                ...
                if (c != -1) {
                    if (c == '&' || c == '%' || c == '<' || c == ']') {
                        ...
                        String key = scanningTextDecl
                            ? "asp.error.xml.invalidCharInTextDecl"
                            : "asp.error.xml.invalidCharInXMLDecl";
        ...
        if (!skipChar(quote)) {
            reportFatalError(scanningTextDecl ?
                             "asp.error.xml.closeQuoteMissingInTextDecl"
                             : "asp.error.xml.closeQuoteMissingInXMLDecl",
                             name);
        }
    ...
    private void scanPIData(String target, XMLString data) 
        ...
        if (target.length() == 3) {
            ...
            if (c0 == 'x' && c1 == 'm' && c2 == 'l') {
                err.aspError("asp.error.xml.reservedPITarget");
            }
        }
        ...
        if (!skipSpaces()) {
            if (skipString("?>")) {
                ...
                err.aspError("asp.error.xml.spaceRequiredInPI");
            }
        }
            ...
            do {
                ...
                if (c != -1) {
                    if (XMLChar.isHighSurrogate(c)) {
                        ...
                        err.aspError("asp.error.xml.invalidCharInPI",
    ...
    private boolean scanSurrogates(XMLStringBuffer buf)
        ...
        if (!XMLChar.isLowSurrogate(low)) {
            err.aspError("asp.error.xml.invalidCharInContent",
